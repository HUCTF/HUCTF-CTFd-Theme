  ...
  $.get(script_root + "/api/v1/challenges", function(response) {
    ...
    for (var i = challenges.length - 1; i >= 0; i--) {
      ...
      if ($.inArray(challenges[i].category, categories) == -1) {
        ...
        var categoryside = $(
          "" +
            '<div id="{0}" class="pt-3">'.format(categoryid)+
            '<div class="category-header col-md-12 mb-3">' +
            "</div>" 
        );
        categoryside
          .find(".category-header")
          .append($('<h4><a id="{0}" class="category-a">'.format(category) + category + "</a></h4>"));  

        $("#challenges-board").append(categoryrow);
        $("#challenges-side").append(categoryside);
      }
    }
    ...
    for (var i = 0; i <= challenges.length - 1; i++) {
      ...
      var chalwrap = $(
        "<div id='{0}' class='col-md-3 d-inline-block chanllenge-border'></div>".format(chalid)
      );
      ...
      var chalscore = $("<span>{0} Pionts</span>".format(chalinfo.value));
